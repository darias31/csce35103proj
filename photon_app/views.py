...
@csrf_exempt  # Disable CSRF for this route
def add_player(request):
    if request.method != 'POST':
        return JsonResponse({"error": "Invalid HTTP method"}, status=405)

    try:
        player_data = json.loads(request.body.decode('utf-8'))
    ...
    try:
        ...
        if existing_player:
            return JsonResponse({"error": "Player ID or Equipment ID already exists"}, status=400)
        ...
        return JsonResponse({"error": "Failed to add player to the database"}, status=500)
    finally:
        conn.close()
    ...
    try:
        ...
        return JsonResponse({"error": "Failed to broadcast equipment ID"}, status=500)

    return JsonResponse({"message": "Player added successfully"}, status=201)
